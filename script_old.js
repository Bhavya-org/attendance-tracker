...
// Server sync simulation using localStorage with a shared key
const SERVER_SYNC_KEY = 'attendance_server_sync';
const EMPLOYEE_LIST_KEY = 'attendance_employee_list';

// Save attendance to "server" (shared localStorage)
function saveAttendanceToServer() {
    ...
    localStorage.setItem(SERVER_SYNC_KEY, JSON.stringify(serverData));
    localStorage.setItem(EMPLOYEE_LIST_KEY, JSON.stringify(employees));
    
    // Show sync feedback
    const syncIndicator = document.getElementById('syncStatus')?.querySelector('.sync-indicator');
    if (syncIndicator && currentMode === 'employee') {
        const originalText = syncIndicator.textContent;
        const originalClass = syncIndicator.className;
        syncIndicator.textContent = '✅ Attendance saved!';
        syncIndicator.className = 'sync-indicator success';
        setTimeout(() => {
            syncIndicator.textContent = originalText;
            syncIndicator.className = originalClass;
        }, 2000);
    }
    
    console.log('Attendance saved to server:', serverData);
}
...
function syncAttendanceData() {
    try {
        ...
        const serverData = localStorage.getItem(SERVER_SYNC_KEY);
        const serverEmployees = localStorage.getItem(EMPLOYEE_LIST_KEY);
        ...
        if (serverData) {
            ...
            if (data.date === today) {
                ...
                Object.keys(serverAttendance).forEach(name => {
                    if (serverAttendance[name] !== 'not-set' && 
                        attendance[name] !== serverAttendance[name]) {
                        attendance[name] = serverAttendance[name];
                        hasUpdates = true;
                    }
                });
                ...
                if (hasUpdates) {
                    console.log('Synced attendance from server:', data);
                }
            }
        }
        
        // If no server data or different day, ensure we have employee list
        if (serverEmployees) {
            const empList = JSON.parse(serverEmployees);
            if (empList.length > 0) {
        ...
        if (syncIndicator) {
            if (hasUpdates) {
                syncIndicator.textContent = '✅ Data updated!';
                syncIndicator.className = 'sync-indicator success';
                setTimeout(() => {
                    syncIndicator.textContent = '🔄 Auto-syncing every 10 seconds';
                    syncIndicator.className = 'sync-indicator';
                }, 3000);
            } else {
                syncIndicator.textContent = '✅ Data in sync';
                syncIndicator.className = 'sync-indicator success';
                setTimeout(() => {
                    syncIndicator.textContent = '🔄 Auto-syncing every 10 seconds';
                    syncIndicator.className = 'sync-indicator';
        ...
        const syncIndicator = document.getElementById('syncStatus')?.querySelector('.sync-indicator');
        if (syncIndicator) {
            syncIndicator.textContent = '⚠️ Sync error - trying again...';
            syncIndicator.className = 'sync-indicator';
        }
    }
    return false;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setInterval(() => {
        const synced = syncAttendanceData();
        if (synced) {
            renderEmployeeList();
            updateSummary();
        }
    ...
    // Save employee list to server on startup
...
function setAttendance(name, status) {
    ...
    // Always save to server sync for cross-mode sharing
    saveAttendanceToServer();
...
function saveData() {
    ...
    // Also save to server sync for cross-mode sharing
    saveAttendanceToServer();
