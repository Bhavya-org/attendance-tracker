...
function createEmployeeCard(name) {
    ...
    // In employee mode, highlight selected employee
    ...
    const canEdit = !isEmployeeMode || selectedEmployee === name;
    
    card.innerHTML = `
        <div class="employee-name">${name}</div>
        ${currentStatus === 'not-set' && isEmployeeMode ? 
            '<div style="color: #ff6b6b; font-size: 0.9rem; margin-bottom: 10px;">⏰ Attendance not marked</div>' : ''}
...
function switchMode(mode) {
    ...
    document.getElementById('attendanceTitle').textContent = 
        mode === 'manager' ? "Today's Attendance" : "All Team Members Status";
...
function showEmployeeWelcome() {
    ...
    welcomeDiv.innerHTML = `
        <h2>👋 Welcome!</h2>
        <p>Please find your name below and mark your attendance for today.</p>
        <p>If you can't find your name, please contact your manager.</p>
    `;
...
function setAttendanceAsEmployee(name, status) {
    ...
    const statusText = status === 'present' ? 'Present in Office' :
                      status === 'wfh' ? 'Work From Home' : 'Absent';
